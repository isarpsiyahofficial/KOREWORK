...
subprocess.check_call([sys.executable,str(WB/'post_v4825_idle_cpu_fix.py'),str(out)],cwd=ROOT)
subprocess.check_call([sys.executable,str(WB/'post_v4826_rogue_skill_extension.py'),str(out)],cwd=ROOT)
h=hashlib.sha256(out.read_bytes()).hexdigest().upper()
(WB/'v4825-final-source-sha.txt').write_text(h+'\n',encoding='utf-8')
print('V4826_FINAL_SOURCE_SHA256='+h)
