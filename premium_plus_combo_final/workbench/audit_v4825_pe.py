...
release=WB/'PremiumPlusCombo-v4.8.25-FINAL.exe'
def pe(path):
...
ci,ri=imp(control),imp(release);extra=sorted(ri-ci);missing=sorted(ci-ri)
...
print('EXACT_IMPORT_SURFACE_MATCH=PASS');print('FINAL_EXE_SHA256='+hashlib.sha256(rb).hexdigest().upper())
