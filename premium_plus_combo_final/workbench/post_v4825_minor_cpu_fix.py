...
UINT MinorSendInputsLowCpu(const INPUT* inputs,UINT count,LONGLONG freq){
  ...
  while(done<count){
    ...
    MinorDelayUs(kMinorNativeGapUs,freq);
  }
  return done;
}
'''
...
sender=s[span(s,'UINT MinorSendInputsLowCpu(')[0]:span(s,'UINT MinorSendInputsLowCpu(')[1]]
