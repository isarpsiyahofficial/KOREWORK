...
public:
    ...
    [[nodiscard]] std::vector<T> readVector(std::size_t count) {
        ...
        if (count > remaining() / sizeof(T)) {
            throw std::runtime_error("Binary vector exceeds remaining file data: " + path_.string());
        }
        std::vector<T> values(count);
        if (!values.empty()) {
            readExact(reinterpret_cast<char*>(values.data()), values.size() * sizeof(T));
        }
        return values;
    }
    ...
    [[nodiscard]] std::vector<std::byte> readBytes(std::size_t count) {
        if (count > remaining()) {
            throw std::runtime_error("Binary byte range exceeds remaining file data: " + path_.string());
        }
        std::vector<std::byte> bytes(count);
        if (!bytes.empty()) {
            readExact(reinterpret_cast<char*>(bytes.data()), bytes.size());
        }
        return bytes;
    }
    ...
    void skip(std::uint64_t count) {
        if (count > remaining()) {
            throw std::runtime_error("Binary skip exceeds remaining file data: " + path_.string());
        }
        stream_.seekg(static_cast<std::streamoff>(count), std::ios::cur);
        if (!stream_) {
            throw std::runtime_error("Unable to seek binary file: " + path_.string());
        }
    }
    ...
    [[nodiscard]] std::uint64_t position() {
        ...
        if (current < 0) {
            throw std::runtime_error("Unable to determine binary position: " + path_.string());
        }
...
private:
    void readExact(char* destination, std::size_t count) {
        if (count > remaining()) {
            throw std::runtime_error("Unexpected end of binary file: " + path_.string());
        }
        stream_.read(destination, static_cast<std::streamsize>(count));
        if (stream_.gcount() != static_cast<std::streamsize>(count)) {
            throw std::runtime_error("Short binary read: " + path_.string());
        }
    }
